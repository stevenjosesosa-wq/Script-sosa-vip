...
def hex_a_rgba(hex_str):
    """ Convierte formato #RRGGBB a valores RGBA flotantes de Unity (0.0 a 1.0) """
...
def procesar_archivos(datos):
    ...
    # Convertimos los colores hexadecimales elegidos a formato flotante de Unity
    ...
    for archivo in archivos:
        ...
        if os.path.isfile(path_origen):
            try:
                # Intentamos leer el dump como JSON
                with open(path_origen, 'r', encoding='utf-8') as f:
                    data = json.load(f)

                # Si el dump contiene la estructura de propiedades m_SavedProperties -> m_Colors
                if "m_SavedProperties" in data and "m_Colors" in data["m_SavedProperties"]:
                    m_colors = data["m_SavedProperties"]["m_Colors"]
                    
                    # Modificamos los valores RGBA correspondientes en el JSON
                    for item in m_colors:
                        prop_nombre = item[0]
                        if prop_nombre in ["_Color_Tex", "_Color_DisTex", "_Mask_Dis", "_TintColor"]:
                            item[1] = rgba_holo
                        elif prop_nombre in ["_OutlineColor"]:
                            item[1] = rgba_antena

                    # Guardamos la estructura JSON modificada
                    with open(path_destino, 'w', encoding='utf-8') as f:
                        json.dump(data, f, indent=4)
                    
                    modificados += 1
                else:
                    # Si no cumple la estructura JSON esperada, se copia de respaldo
                    shutil.copy2(path_origen, path_destino)
                    modificados += 1

            except Exception:
                # Si falla el parseo JSON (por ser binario u otro formato), se copia directamente
                shutil.copy2(path_origen, path_destino)
                modificados += 1

    return True, f"¡Modificación exitosa! Se editaron {modificados} dumps JSON con los colores convertidos."
