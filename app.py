...
def procesar_archivos(datos):
    ...
    if opcion == 13:
        ...
        return True, f"¡Renombrado exitoso! Se guardaron {modificados} archivos con la base '{nuevo_nombre}_X'."
    ...
    if opcion == 14:
        ...
        return True, "🔍 REPORTE DE CHECKSUM CRC32:\n\n" + "\n".join(resumen_crc)

    # OPCIONES 1 A 12: Procesamiento de Dumps, Colores e Inspección de Texturas
    if not os.path.exists(destino):
        os.makedirs(destino, exist_ok=True)
    ...
    for archivo in archivos:
        ...
        if os.path.isfile(path_origen):
            try:
                ...
                # Extraer info de texturas vinculadas
                texturas_detectadas = []
                if "m_SavedProperties" in data and "m_TexEnvs" in data["m_SavedProperties"]:
                    for tex_item in data["m_SavedProperties"]["m_TexEnvs"]:
                        nombre_tex = tex_item[0]
                        texturas_detectadas.append(nombre_tex)
                ...
                if "m_SavedProperties" in data and "m_Colors" in data["m_SavedProperties"]:
                    ...
                    tex_info = ", ".join(texturas_detectadas) if texturas_detectadas else "General/Holo"
                    reporte_texturas.append(f"✔ {archivo} ➔ Textura: [{tex_info}]")
                else:
                    shutil.copy2(path_origen, path_destino)
                    modificados += 1
    ...
    resumen_final = f"✅ ¡Modificación exitosa! ({modificados} dumps procesados)\n\n"
    if reporte_texturas:
        resumen_final += "🎨 TEXTURAS MODIFICADAS:\n" + "\n".join(reporte_texturas[:15])

    return True, resumen_final
...
<script>
    ...
    function ejecutarAccion() {{
        ...
        statusDiv.innerText = '⏳ Procesando y leyendo texturas...';
